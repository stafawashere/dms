...
export async function getDashboardData() {
   ...
   const [
      ...
      prisma.user.count({ where: { role: Role.RESELLER, active: true } }),
      prisma.sale.findMany({ where: { status: SaleStatus.APPROVED }, include: { product: true, reseller: { select: { name: true } } } }),
      prisma.sale.findMany({
         where: { status: SaleStatus.APPROVED, createdAt: { gte: sevenDaysAgo } },
   ...
   const pendingSalesCount = await prisma.sale.count({ where: { status: SaleStatus.PENDING } });
